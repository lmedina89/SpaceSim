...
if (!html.includes('Universe Lab v0.1.5.5.2')) throw new Error('Shell version is not v0.1.5.5.2.');
if (!html.includes('<div class="brand">UNIVERSE LAB <span>v0.1.5.5.2</span></div>')) throw new Error('Visible top-left build badge is not v0.1.5.5.2.');
if (!html.includes('DEEPFRAME-1552')) throw new Error('DEEPFRAME-1552 build marker missing.');
if (pkg.version !== '0.1.5.5.2') throw new Error('package.json version mismatch.');
...
if (!html.includes('./styles.css?v=1552') || !html.includes('./src/main.js?v=1552')) throw new Error('Build-version cache-busting tags missing.');
...
if (!main.includes("./app/app.js?v=1552")) throw new Error('Main-to-app cache-busting import missing.');
if (!app.includes("../render/threeRenderer.js?v=1552") || !app.includes("../ui/hud.js?v=1552") || !app.includes("../ui/systemMap.js?v=1552")) throw new Error('App cache-busting imports missing.');
if (!renderer.includes("./cockpitView.js?v=1552")) throw new Error('Cockpit renderer cache-busting import missing.');
...
if (versionJson.buildMarker !== 'DEEPFRAME-1552') throw new Error('VERSION.json build marker mismatch.');
if (!html.includes('<option value="5000">5,000 c · DEEP</option>')) throw new Error('DEEP FRAME selector option missing.');
if (!transitDrive.includes("{ multipleC: 5000, label: '5,000 c · DEEP' }") || !transitDrive.includes('remaining > PHYSICS.AU * 50')) throw new Error('Range-gated DEEP FRAME tier missing.');
if (!String(versionJson.deepFrameTier || '').includes('beyond 50 AU') || !String(versionJson.deepFrameTier || '').includes('swept massive-body guard')) throw new Error('VERSION.json DEEP FRAME boundary missing.');
