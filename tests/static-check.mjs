...
if (!html.includes('Universe Lab v0.1.4.7')) throw new Error('Shell version is not v0.1.4.7.');
if (!html.includes('ROTASTRO-147')) throw new Error('ROTASTRO-147 build marker missing.');
if (pkg.version !== '0.1.4.7') throw new Error('package.json version mismatch.');
...
if (!html.includes('./styles.css?v=147') || !html.includes('./src/main.js?v=147')) throw new Error('Build-version cache-busting tags missing.');
...
for (const id of ['velocityMarker','progradeButton','retrogradeButton','turnBurnButton','transitToggle','transitPanel','transitTargetSource','transitTier','transitEngage','frameQuick','thrustValue','cockpitOverlay','cockpitStatus','cockpitToggle','ascentDiagnostic']) if (!html.includes(`id="${id}"`)) throw new Error(`Navigation/transit/cockpit control missing: ${id}`);
...
for (const id of ['landTarget','surfaceLandButton','mapLandAction','surfaceHud','surfaceWorldName','surfaceBiome','surfaceGravity','surfaceTemperature','surfaceAtmosphere','surfaceCoords','surfaceDiscoveries','surfaceNearest','surfaceScanStatus','surfaceScanButton','surfaceSprintButton','surfaceSaveButton','surfaceTakeoffButton','surfaceMovePad','surfaceForward','surfaceBack','surfaceLeft','surfaceRight','surfaceWeather','surfaceWind','surfaceShipDistance','surfaceClock','surfaceWeatherStatus','surfaceRegionSelect','surfaceRegionLabel','surfaceHudToggle','surfaceHudDetails','surfaceShipCompact','surfacePhase','surfaceSkyClock','surfaceRotation']) if (!html.includes(`id=\"${id}\"`)) throw new Error(`Surface foundation control missing: ${id}`);
...
for (const token of ['captureBodyFixedSurfaceAnchor','bodyFixedDirectionToInertial','inertialDirectionToBodyFixed','surfaceTangentBasis','rotationAngleAt']) if (!planetaryRotation.includes(token)) throw new Error(`Planetary rotation token missing: ${token}`);
...
if (!main.includes("./app/app.js?v=147")) throw new Error('Main-to-app cache-busting import missing.');
if (!app.includes("../render/threeRenderer.js?v=147") || !app.includes("../ui/hud.js?v=147")) throw new Error('App cache-busting imports missing.');
if (!renderer.includes("./cockpitView.js?v=147")) throw new Error('Cockpit renderer cache-busting import missing.');
...
if (versionJson.buildMarker !== 'ROTASTRO-147') throw new Error('VERSION.json build marker mismatch.');
