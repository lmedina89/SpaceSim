...
test('fragment generator never allocates more resolved fragment mass than the impactor mass', () => {
  ...
  assert.ok(resolved <= impactor.mass * 0.421);
  assert.ok(generated.fragments.length <= 6);
});
...
test('energetic asteroid-to-planet resolution conserves represented gravitational mass', () => {
  ...
  const resolution=resolveImpact({a:target,b:impactor,relativeVelocity:[-20000,0,0],relativeSpeed:20000,contactNormal:[1,0,0],timeSeconds:123},{maxGravityFragments:6});
...
test('low-energy asteroid collision bounce conserves total linear momentum', () => {
  ...
  const r=resolveImpact({a,b,relativeVelocity:[-10,0,0],relativeSpeed:10,contactNormal:[1,0,0]},{maxGravityFragments:6});
