...
test('transit tiers are explicitly coordinate-rate multiples of c, not local ship velocity',()=>{
  assert.equal(normalizeTransitMultiple(87),100);
  assert.equal(transitSpeedMps(100),100*PHYSICS.C);
});

test('transit arrival envelope leaves physical BOOST braking room for preserved local delta-v',()=>{
  const ship=new ShipDynamics();
  ship.velocity.set([23_000_000,0,0]);
  ...
  const arrival=transitArrivalDistanceMeters(ship,target,5000);
  const pureBrake=23_000_000**2/(2*5000);
  assert.ok(arrival>pureBrake,`${arrival} <= ${pureBrake}`);
});

test('transit automatically steps down near target',()=>{
  const arrival=1e8;
...
test('transit position step never overshoots its arrival envelope',()=>{
...
test('swept transit guard catches a massive body crossed between frames',()=>{
...
test('transit engagement clearance rejects starting inside a body safety envelope',()=>{
